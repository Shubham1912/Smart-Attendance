...
def output(filename, sheet,num, name, present):
    my_file = Path('firebase1/attendence_files/'+filename+str(datetime.now().date())+'.xls');
    if my_file.is_file():
        rb = open_workbook('firebase1/attendence_files/'+filename+str(datetime.now().date())+'.xls');
        book = copy(rb);
        sh = book.get_sheet(0)
        # file exists
    else:
    ...
    book.save('firebase1/attendence_files/'+fullname)
    return fullname;
